    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.grayFour }}>
            <View style={styles.container}>
                ...
                {languages.map((item) => (
                    <TouchableOpacity key={item} style={styles.menuButton} onPress={() => setSelectedLang(item)}>
                        <View style={styles.circeOut}>
                            {selectedLang === item ? <View style={styles.circeInner} /> : null}
                        </View>
                        <Text numberOfLines={1} style={styles.menuTitle}>{item}</Text>
                    </TouchableOpacity>
                ))}
            </View>
            <MainButton title="Save and restart" containerStyle={styles.saveButton} labelStyle={{ color: "#FFFFFF" }} />
        </SafeAreaView>
    );
});
